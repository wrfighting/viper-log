...
let logConfig = {
    appenders: {
        ...
        publiclog: {
            type: 'dateFile',
            filename: 'log/publiclog/public.log',
            pattern: 'yyyyMMddhh',
            daysToKeep: 60,
            layout: {
                type: 'pattern',
...
class ViperLog {
    ...
    formatPrint = utils.formatPrint
    getExec = utils.getExec
}
...
ViperLog.initLogKoa = function (options = {}) {
    return async function (ctx, next) {
        ...
        ctx.logger = new ViperLog({
            traceid: utils.getTraceId(headers),
            spanid: utils.getSpanId(headers),
        ...
        if (
            options &&
            ((options.skip && options.skip.includes(ctx.url)) ||
                (options.skipMethod && options.skipMethod.includes(ctx.method)))
        ) {
            return next()
        }
        let start = new Date()
...
ViperLog.initLogExpress = function (options = {}) {
    return function (req, res, next) {
        ...
        if (
            options &&
            ((options.skip && options.skip.includes(req.url)) ||
                (options.skipMethod && options.skipMethod.includes(req.method)))
        ) {
            return next()
        }
        let start = new Date()
